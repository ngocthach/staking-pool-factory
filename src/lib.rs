...
impl StakingPoolFactory {
    ...
    pub fn create_staking_pool(
        ...
        const GAS_TO_NEAR_RATE: u128 = 1_000_000_000_000_000_000_000_000;
        let _not_enough_deposit_message = format!("Not enough attached deposit to complete staking pool creation. It must be greater than {} Near", &(MIN_ATTACHED_BALANCE/GAS_TO_NEAR_RATE));

        assert!(
            env::attached_deposit() >= MIN_ATTACHED_BALANCE,
            "{}",
            _not_enough_deposit_message
        );
